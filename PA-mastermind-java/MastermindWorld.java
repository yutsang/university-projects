...
/**
 * MastermindWorld - Main game logic for the Mastermind Java game using Greenfoot.
 * Handles world setup, secret generation, scoring, and save/load functionality.
 * 
...
public class MastermindWorld extends GWorld
{
    ...
    // Constants representing different objects on the game board
    ...
    // The secret code, randomized at the beginning
    public static String secret = "wde";

    // High score array (optional usage)
    public static int highScore[] = {0, 0, 0};
    ...
    /**
     * Constructor: Initializes the world and generates a new secret.
     */
    public MastermindWorld() throws Exception
    {           
        initialize(); 
        generateSecret(); // Comment out for testing with fixed secret
    }

    /**
     * Initializes the game board with blocks and judges.
     */
    public static void initialize()
    {  
        removeAllObjects();
        // Fill the first line with blocks
        int width = GWorld.width();
        for (int i = 0; i < width; i++) {
            Block newBlock = new Block();
            GWorld.addOneObject(newBlock, i, 0);
        }
        // Place blocks in the center column
        int height = GWorld.height();
        for (int j = 1; j < height; j++) {
            Block newBlock = new Block();
            GWorld.addOneObject(newBlock, 3, j);
        }
        // Place judges
